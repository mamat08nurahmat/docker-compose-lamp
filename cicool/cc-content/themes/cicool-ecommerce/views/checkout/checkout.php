        ...
        <div class="box-checkout">
          ...
          <div class="box-checkout-content">
            <div class="row">
              <div class="col-md-12">
                <?php if (!$address): ?>
                <a href="<?= base_url('member/shipping/add') ?>" class="btn btn-default"><span class="fal fa-plus-square"></span> Add shipping address</a>
                <?php else: ?>
                  <input type="hidden" value="<?= $address->id ?>" name="shipping_address_id">
                  <div class="col-md-4"><span class="checkout-addr"><?= _ent($address->label) ?> <?= _ent($address->phone) ?></span></div>
...
<script>
  $(function(){
    $('.delivery-method-item').click(function(event) {
      $(this).find('input[type="radio"]').prop('checked', 1);
    });
    $('.payment-method-item').click(function(event) {
      $(this).find('input[type="radio"]').prop('checked', 1);
    });
  })
</script>
